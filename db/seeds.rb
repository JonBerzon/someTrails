...
revEm1 = Review.create!(user_id: user1.id, activity: "Backpacking", trail_id: emerald.id, date: "01-01-2021", description: "Gorgeous! Lakes look mostly frozen, though I didn’t venture out on them. Traction highly recommended. Saw a few people without but they weren’t moving fast or at all. Arrived at the main parking lot at 8:50ish (15 minute wait at the main gate) and there was plenty of parking. But all lots full at 10:45 (except shuttle lot).", rating: 5)
revEm2 = Review.create!(user_id: user2.id, activity: "Hiking", trail_id: emerald.id, date: "2021-01-01", description: "Great hike. Get there early 7am if you want to beat the busy time and catch a sunrise. I wore spikes and I'm glad I did but there were definitely people doing it without them. Like a previous user said it's mostly at the beginning and end. Enjoy!", rating: 5)
revEm3 = Review.create!(user_id: user3.id, activity: "Hiking", trail_id: emerald.id, date: "2021-01-01", description: "Have done this hike several times over the years and it never disappoints! Hiked it this past Sunday, 10/31. Less crowded this time of year, plenty of parking at Bear Lake trailhead. Lots of ice on the trail. People were doing it in sneakers, but I would highly recommend bringing microspikes. I wore mine the entire hike,  except at the very beginning and end. Got caught in a bit of a snow storm leaving Emerald Lake, and ran into a momma moose and two moose babies on the trail at Dream Lake on the way up.", rating: 5)
revEm4 = Review.create!(user_id: user4.id, activity: "Walking", trail_id: emerald.id, date: "2021-01-01", description: "Maybe the most touristy hike in the park, but that doesn't make it bad by any means. Plenty of traffic when the weather is decent and really beautiful paths and views. Each lake is a treat to stop at and enjoy and nice for a break as well. You can reach Haiyaha from this trail and you definitely should. Great payoff at the end, highly recommend this hike. ", rating: 4)
revEm5 = Review.create!(user_id: user5.id, activity: "Hiking", trail_id: emerald.id, date: "2021-01-01", description: "Been here a couple of times - the most recent trip was back in late August. The trail is really straight forward and super busy when there's no snow. All the lakes are beautiful. My favorite of the lot being Dream Lake and the mini waterfall right before it. Nymph lake is pretty quiet than the other two. Great views of Long's peak and the other mountains from the trail. ", rating: 4)

revSk1 = Review.create!(user_id: user5.id, activity: "Rock Climbing", trail_id: sky.id, date: "2021-01-01", rating: 5, description: "Started around 6:30 AM, not too cold until we got to the scramble before Lake of Glass — and that was a scramble! Verrrry windy at the top. We had spikes and poles, and thank goodness for that because we would not have made it to the top. We didn’t put the spikes on until about a mile in from the trailhead, but they were useful all the way up and back. My first ever difficult hike, and it was the longest. I am not a seasoned hiker, but it was the most fun I’ve ever had, even if it was the most difficult thing I’d done!" )
revSk2 = Review.create!(user_id: user4.id, activity: "Snowshoeing", trail_id: sky.id, date: "2021-01-01", rating: 5, description: "One of, if not the best trail in RMNP. Spikes required for this time of year once you get past loch lake but the trail is well marked all the way through. Waist high snow in some areas at the top but definitely still hikeable." )
revSk3 = Review.create!(user_id: user3.id, activity: "Hiking", trail_id: sky.id, date: "2021-01-01", rating: 5, description: "Amazing hike but you’re going to need spikes pretty much from the parking lot at this time of year. The scramble up the waterfall is frozen, making it even more challenging. There are two lakes after climbing the waterfall, make sure you keep going! It’s not well marked but so worth it!" )
revSk4 = Review.create!(user_id: user2.id, activity: "Rock Climbing", trail_id: sky.id, date: "2021-01-01", rating: 4, description: "Climbed to the top of the waterfall which required spikes. After about 10 minutes at the top we had to turn around due to the winds. (over 50mph gusts easily). Great view at the top though. " )
revSk5 = Review.create!(user_id: user1.id, activity: "Hiking", trail_id: sky.id, date: "2021-01-01", rating: 4, description: "Would recommend arriving early, we got to the trailhead around 8:15 and there were only ~3 spots still available. You don't really need microspikes until the Loch and then I'd definitely recommend. The waterfall to Lake of Glass is extremely icy in most areas making the ascent/descent difficult and slippery. You don't need spikes after Lake of Glass since the area is bathed in sunlight and fairly ice/snow free." )

revAl1 = Review.create!(user_id: user2.id, activity: "Hiking", trail_id: alberta.id, date: "2021-01-01", rating: 5, description: "Great hike. Family friendly. A little bit icy right now. But well worth it and definitely manageable. Views of the aspens and the falls are top notch!" )
revAl2 = Review.create!(user_id: user1.id, activity: "Walking", trail_id: alberta.id, date: "2021-01-01", rating: 3, description: "It was ok, I wish I would have done it first that day. We’d already done Bear and Dream and the trail was a little more demanding than I expected (had a toddler we were carrying at this point). Definitely not a stroll like Bear Lake. Went on a Sunday so the falls were so crowded we couldn’t really get any good pictures or even sit where we could see it.  I would do this hike again on a weekday, or early morning next time. The falls were a bit anti-climactic after the lakes and by far the most crowded hike we did that day. It’s a beautiful trail though, especially during leaf season." )
revAl3 = Review.create!(user_id: user4.id, activity: "Nature trips", trail_id: alberta.id, date: "2021-01-01", rating: 4, description: "Fun moderate hike. Dream Lake is probably beautiful, but it was fogged in so I’ll never know. Well maintained, very busy. No parking available at the trailhead; we took the shuttle." )
revAl4 = Review.create!(user_id: user3.id, activity: "Hiking", trail_id: alberta.id, date: "2021-01-01", rating: 4, description: "Easy hike. Beautiful view of a canyon just before the waterfall. Tons of wildlife on the trail. If the Glacier lot is full and you have to park at Bear Lake, park as close to the road as you can. There is a horse trail that connects to the main trail. It will shave 0.1 off your route. " )
revAl5 = Review.create!(user_id: user5.id, activity: "Hiking", trail_id: alberta.id, date: "2021-01-01", rating: 5, description: "Beautiful hike. In the summer best to arrive by 7 am to find a parking space and to beat the crowds. We hiked to the waterfall then walked back to Bear Lake, Nymph Lake and Emerald Lake, which Emerald lake is spectacular." )
 
revGe1 = Review.create!(user_id: user5.id, activity: "Hiking", trail_id: gem.id, date: "2021-01-01", rating: 4, description: "Great views throughout the hike. Lake is underwhelming. But with this trail it is not the destination but the journey that matters. Climbs at the beginning and end middle relatively level. Was snow and ice free Nov7" )
revGe2 = Review.create!(user_id: user3.id, activity: "Hiking", trail_id: gem.id, date: "2021-01-01", rating: 4, description: "Pretty decent trail. Views are good at some points but the lake was a little disappointing. Very windy at the top. If there weren’t better trails nearby this one would be great but knowing emerald lake trail is nearby makes this seem like a waste of time in my opinion." )
revGe3 = Review.create!(user_id: user2.id, activity: "Fishing", trail_id: gem.id, date: "2021-01-01", rating: 4, description: "Great short hike with good workout and stunning scenery.  Lake was windy-rest of the trail mostly sheltered.  Plenty of parking at 8:45 am but almost full by 11:30 am on a Friday.  Definitely getting busy toward noon time. Expect rocky and steep trails in parts. Took me about 2 hours and 40 minutes." )
revGe4 = Review.create!(user_id: user1.id, activity: "Walking", trail_id: gem.id, date: "2021-01-01", rating: 5, description: "So many beautiful vistas on this hike, highly recommend trekking poles as the upper steps are pretty steep. The lake initially does appear as underwhelming, but it’s much more striking when taking the trail to the left to the far end of the lake and look south. I would definitely hike this again and allow time to continue us on to Balanced Rock. " )
revGe5 = Review.create!(user_id: user4.id, activity: "Rock climbing", trail_id: gem.id, date: "2021-01-01", rating: 5, description: "Beautiful trails with beautiful views all the way up! Gem Lake was very pretty & very windy. The ascend was tough for these Hoosier lungs, but there was plenty of places to take a quick break. Recommend!!" )

revCh1 = Review.create!(user_id: user2.id, activity: "Hiking", trail_id: chasm.id, date: "2021-01-01", rating: 4, description: "Well maintained trail, quite windy in the last mile up but great weather most of the way. There wasn’t too much ice in the last climb, sun was out and I suppose melted quite a bit. Stayed towards the left on the last climb up to the lake, a little bit more difficult but there wasn’t any ice to deal with. " )
revCh2 = Review.create!(user_id: user3.id, activity: "Rock climbing", trail_id: chasm.id, date: "2021-01-01", rating: 5, description: "Took us 4.7 miles to get to Chasm Lake from the trailhead, 9.15 total hike, 2,631 feet in elevation gain. The trail was clear and dry, but there were A LOT of steps the entire way. The scramble to Chasm was hard… there’s no clear path to the lake (and it was a sheet of ice on one of the potential routes) so we rock climbed up. Before Chasm, you will also see Peacock Pool and some other small water falls. Weather was perfect, not too hot or cold but a little bit windy in certain parts." )
revCh3 = Review.create!(user_id: user1.id, activity: "Hiking", trail_id: chasm.id, date: "2021-01-01", rating: 5, description: "Absolutely stunning! Ice wasn’t too bad and spikes weren’t necessary! Definitely recommend gloves with rubber to help with rock scramble at the end! Saw a couple moose on the trail! Super cool but please be careful!! Arrived at 8:30 am at trailhead and plenty of parking left! :) " )
revCh4 = Review.create!(user_id: user4.id, activity: "Snowshoeing", trail_id: chasm.id, date: "2021-01-01", rating: 4, description: "Wind and snow to maneuver from last nights snowfall. Drifts 4-8” some ice at the top. Wind gusts 40 mph+ which became sustained after 3pm. A few frozen falls on the way up to the Alpine hike, not much to look at except the stairs until you get above the treeline. If you can make it that far its a big elevation gain but you are rewarded with incredible views.  Worth it" )
revCh5 = Review.create!(user_id: user5.id, activity: "Snowshoeing", trail_id: chasm.id, date: "2021-01-01", rating: 4, description: "Got to trail head about 5:45 am, only a few cars in parking lot.  Started the hike around 6:30 when it was light enough to see without headlamp.  Great hike, well maintained and marked trail.  Somewhere between moderate and difficult.  Lots of ice on scramble at end but still doable, even for us without crampons.  It was pretty cold at start and up top.  3 hours up, 2 hours back.  Parking lot was still only half full when we were leaving.  " )
...
grown = Condition.create!(name: "Overgrown")
